...
\begin{align*}
\forall x ( S(x) = \bigcup \{x, \{ x \} \} )
\end{align*}
\end{defn}

\begin{prop}
\label{Proposition:pairwise_union}
Pairwise union.
\begin{align*}
\forall A \forall B \forall x (x \in \bigcup \{ A,B \} \iff ((x \in A) \lor (x \in B)))
\end{align*}
Proof:
\begin{align*}
& \forall A \forall B \forall x ( \\
& \quad && &&  x \in  \bigcup \{ A,B \} \\
...
= & \bigcup \{0, \{ 0 \} \} && \text{Definition \ref{Definition:successor}} \\
= & \bigcup \{\emptyset, \{ \emptyset \} \} && \text{Definition \ref{Definition:zero}} \\
\end{align*}
\end{defn}